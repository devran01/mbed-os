...
def test_psa_target_attributes():
    ...
    for tar in psa_targets:
        # Check that all PSA secure target names ends with _S
        assert(tar.name.endswith('_S'))
        try:
            tar_inherits = "PSA" in tar.inherits[0]
        except AttributeError:
            msg = """Armv8-M and dual-core PSA targets MUST inherit from the
                     generic PSA target that corresponds to that target's
                     architecture
                  """
            assert 0, msg
        assert(tar_inherits == True), msg
